...
Scott TODO: remove dependencies on the urbansim libraries.
...
from urbansim.models import util as us_util
...
class AllocationModel(object):
    ...
    def allocate(self, data, year):
        ...
        if self.target_col in data and (self.as_delta or year not in self.amounts_df.index):
            ...
            results = pd.Series(np.zeros(len(data), dtype='int64'), index=data.index)
            target_dtype = results.dtype
                ...
                if self.as_integer:
                    ...
                    if fract_sum > 0:
                        ...
                        idx_to_round_up = np.random.choice(
                            to_round_idx, round_up_cnt.astype("int32"), False, fract_w)
                        a[idx_to_round_up] += 1
        ...
        # a = results.isnull()
        # print results[a]
        # Hanyi, for construction jobs, some new buildings get NA, this is a temp fix using fillna
        return results.fillna(0).astype(target_dtype)
...
class AgentAllocationModel(object):
    ...
    as_delta: bool, optional, it will be passed to the allocation model, default False
        If True, the allocation will assume that the amount being
        distributed represents change. The resulting series
        will retain the original column values and will increment
        the column by the allocated change. If False, the allocation
